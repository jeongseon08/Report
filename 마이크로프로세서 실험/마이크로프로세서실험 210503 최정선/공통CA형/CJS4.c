...
unsigned char fnd_db[10]= {~0x77, ~0x41, ~0x3b,~0x5b,~0x4d,~0x5e,~0x7c,~0x43,~0x7f,~0x4f};// 0부터 9까지 선언 (실험하고자 하는 세그먼트가 CA형이므로 not을 붙여줌.)
...
int main()
{
	...
	//1의 자리를 디스플레이
		...
		PORTE = fnd_db[a];
		PORTC = ~0xBF; //1의 자리 'display on' 6번 비트 1:on , 0번 포트는 not에 의해 0의 값을 유지한다.
		_delay_ms(10);//숫자를 관찰할 수 있도록 delay부여

		//10의 자리를 디스플레이1
		PORTE = fnd_db[b];
		PORTC = ~0x7F;//10의 자리 'display on' 7번 비트 1:on, 0번 포트는 not에 의해 0의 값을 유지한다.
		_delay_ms(10);//숫자를 관찰할 수 있도록 delay부여
		...
		//1의 자리를 디스플레이
		PORTE = fnd_db[c];
		PORTC = ~0xEF; //1의 자리 'display on' 4번 비트 1:on, 0번 포트는 not에 의해 0의 값을 유지한다.
		_delay_ms(10); //숫자를 관찰할 수 있도록 delay부여

		//10의 자리를 디스플레이
		PORTE = fnd_db[d];
		PORTC = ~0xDF; //10의 자리 'display on' 5번 비트 1:on, 0번 포트는 not에 의해 0의 값을 유지한다.
