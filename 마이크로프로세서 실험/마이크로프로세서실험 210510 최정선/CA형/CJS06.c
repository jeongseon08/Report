...
int main()
{
	...
	while(1)
	{
		...
		PORTC = ~0xEF; //1의 자리 'display on' 6번 비트 1:on,CA형 이므로 not
		_delay_ms(2);//숫자를 관찰할 수 있도록 delay부여

		//10의 자리를 디스플레
		PORTE = ~fnd_db[b];//CA형 이므로 not
		PORTC = ~0xDF;//10의 자리 'display on' 7번 비트 1:on.CA형 이므로 not
		_delay_ms(2);//숫자를 관찰할 수 있도록 delay부여
		...
		PORTC = ~0xBF; //1의 자리 'display on' 4번 비트 1:on,v
		_delay_ms(2); //숫자를 관찰할 수 있도록 delay부여

		//10의 자리를 디스플레이
		PORTE = ~fnd_db[d];//CA형 이므로 not
		PORTC = ~0x7F; //10의 자리 'display on' 5번 비트 1:on,CA형 이므로 not
		_delay_ms(2);//숫자를 관찰할 수 있도록 delay부여
